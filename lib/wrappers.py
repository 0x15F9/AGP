...
class ProcessFrame84SquashRot(gym.ObservationWrapper):
    def __init__(self, env=None):
        super(ProcessFrame84SquashRot, self).__init__(env)
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(84, 84, 1), dtype=np.uint8)

    def observation(self, obs):
        return ProcessFrame84SquashRot.process(obs)
